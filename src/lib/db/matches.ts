...
export async function getSeasonMatchCount(options?: { includeTestSessions?: boolean; seasonId?: string; seasonLockDate?: string | null }): Promise<number> {
  const supabase = await createClient();
  const includeTest = options?.includeTestSessions ?? false;
  ...
  let matchQuery = includeTest
    ? supabase.from("matches").select("*, sessions!inner(is_test_session, season_id, date)", { count: "exact", head: true }).eq("match_type", "regular")
    : supabase
        .from("matches")
        .select("*, sessions!inner(is_test_session, season_id, date)", { count: "exact", head: true })
  ...
  if (seasonLockDate) {
    matchQuery = matchQuery.lte("sessions.date", seasonLockDate);
  }

  let tallyQuery = includeTest
    ? supabase.from("session_tally").select("wins, sessions!inner(is_test_session, season_id, date)")
    : supabase
        .from("session_tally")
        .select("wins, sessions!inner(is_test_session, season_id, date)")
        .eq("sessions.is_test_session", false);
  ...
  if (seasonLockDate) {
    tallyQuery = tallyQuery.lte("sessions.date", seasonLockDate);
  }
