...
export default function SessionScoreboard({ scoreboard, playerId, matchCount }: Props) {
  ...
  return (
    <div className="bg-surface rounded-xl shadow-sm dark:shadow-none dark:ring-1 dark:ring-border px-4 py-3">
      ...
      {scoreboard.length === 0 ? (
        <p className="text-sm text-muted-light">No matches recorded yet.</p>
      ) : (
        <>
          {/* Header */}
                ...
                <div key={p.player_id} className="flex items-center px-1">
                  <span className="flex-1 flex items-center gap-1 min-w-0">
                    <NavLink
                      ...
                      className="text-sm font-medium truncate text-sky-600 dark:text-sky-400 hover:underline active:opacity-60"
                    >
                      {p.name}
                    </NavLink>
                  ...
                  <span className="w-10 text-center text-sm tabular-nums text-text-light">{p.points}</span>
                  <span className="w-12 text-right text-sm text-text-light">{p.matches_played ? `${pct}%` : <span className="text-muted-lighter">—</span>}</span>
                </div>
              );
            })}
          </div>
