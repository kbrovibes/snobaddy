...
export default function IncludeTestToggle({ enabled }: { enabled: boolean }) {
  ...
  return (
    ...
    >
      <span className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out ${enabled ? "bg-sky-500" : "bg-gray-300"}`}>
        <span
